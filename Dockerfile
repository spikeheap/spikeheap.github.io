...
# From https://github.com/mhart/alpine-node/blob/fe6bb865a9de9f787abd189abb6db81f3f645765/Dockerfile
...
# Where we're placing our code
ENV APP_HOME /site
RUN mkdir $APP_HOME
WORKDIR $APP_HOME

#
## Locally cached dependency layer
#
...
# Copy over the app after we've run bundler, bower and NPM, so we don't invalidate the cache
ADD . $APP_HOME
